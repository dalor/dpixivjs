...
const prepareTags = (tags) =>
  tags
    .map(
      (tag) =>
        `<code>${tag.tag}</code>${
          (tag.translation && "(" + tag.translation.en + ")") || ""
        }`
    )
    .join(", ");

const prepareCaption = (pic, data, url, no_page) => {
  return `<a href=\"${dataUrl(data, url)}\">${fixChars(pic.illustTitle)}</a> ${
    !no_page && data.pageCount > 1 ? data.page === undefined ? `(${data.pageCount})` : `(${data.page + 1}/${data.pageCount})` : ""
...
exports.newPic = ({ pic, data, ctx, no_reply_markup, no_page }) => {
  return {
    photo:
      data && data.page > 0
        ? changePage(pic.urls.medium, data.page) ||
          originalToRegular(pic.urls.original, data.page)
        : pic.urls.medium || originalToRegular(pic.urls.original, 0),
...
exports.channelPic = ({ pic, data, ctx }) => {
  return {
    photo:
      data.page > 0
        ? changePage(pic.urls.regular, data.page)
        : pic.urls.regular,
        ...
        [
          ...
          markup.urlButton(
            ...
            `https://t.me/${ctx.botInfo.username}?start=${
              data.page > 0 ? `${data.id}_${data.page}` : data.id
            }`
          ),
        ],
      ])
...
exports.updatePic = ({ data, caption, ctx }) => {
  return {
    photo: originalToRegular(data.url, data.page || 0),
