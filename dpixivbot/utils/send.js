...
exports.sendPic = (ctx, id, page, data) =>
  info({ id }).then((pic) => {
...
exports.sendOneToChannel = (ctx, channelId, data) =>
  info({ id: data.id }).then((pic) => {
    const { photo, reply_markup } = channelPic({ pic, data, ctx });
    return ctx.telegram.sendPhoto(channelId, photo, { reply_markup });
  });

exports.sendFull = (ctx, id, data) =>
  info({ id }).then(async (pic) => {
    const picsCtx = new Array(pic.pageCount)
      .fill()
      .map((_, i) => picCtx(ctx, pic, i, true, true));
...
const picCtx = (ctx, pic, page, no_reply_markup, no_page) =>
  newPic({
    ctx,
    pic,
    data: {
...
const sendPackByOne = (ctx, pics, data) =>
  Promise.all(pics.map((pic) => sendOne(ctx, pic, 0, data)));

const splitToPacks = (pics, pack_size) => {
  const packs = pics.reduce(
    (packs, pic, i) => {
  ...
  if (packs.lastPack.length) return packs.packs.concat([packs.lastPack]);
  else return packs.packs;
};
